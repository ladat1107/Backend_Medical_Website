        ...
        users.forEach(user => {
            ...
            if (user.staffUserData && user.staffUserData.length > 0) {
                ...
                if (department) {
                    ...
                    // Group by department
                    if (!groupedUsers.staffUsers.byDepartment[departmentId]) {
                        groupedUsers.staffUsers.byDepartment[departmentId] = {
                            departmentInfo: {
                                id: departmentId,
                    ...
                    // If user is dean, put in dean field
                    if (staffUser.isDean) {
                        groupedUsers.staffUsers.byDepartment[departmentId].dean = staffUser;
                    } else {
                        groupedUsers.staffUsers.byDepartment[departmentId].staff.push(staffUser);
                    }
                }
            } else {
                // Add to regularUsers
                groupedUsers.regularUsers.push(plainUser);
            }
        });

        // Convert department object to array for easier frontend handling
        const departmentsArray = Object.values(groupedUsers.staffUsers.byDepartment);
        
        const result = {
            regularUsers: groupedUsers.regularUsers,
            staffByDepartment: departmentsArray
        };
